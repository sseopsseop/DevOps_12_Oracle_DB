...
SELECT P.PNO 
	 ...
	 , P.HIREDATE 
	FROM PROFESSOR P
	WHERE P.HIREDATE BETWEEN TO_DATE('19990101', 'YYYYMMDD') AND TO_DATE('19990101', 'YYYYMMDD') + 100
	ORDER BY HIREDATE;

-- 1-6 날짜 함수
-- ROUND(날짜, 날짜 단위) : 지정한 날짜 단위까지의 날짜는 빼오고 나머지 날짜는 초기화, 날짜를 빼올 때 절반이상 지났으면 반올림한다
...
-- TRUNC(날짜, 날짜 단위) : 지정한 날짜 단위까지의 날짜는 빼오고 나머지 날짜는 초기회, 버려지는 날짜는 그냥 초기화
SELECT SYSDATE
	 , TRUNC(SYSDATE, 'YYYY')
	 , TRUNC(SYSDATE, 'MM')
	 , TRUNC(SYSDATE, 'DD')
...
-- LAST_DAY(날짜) : 날짜를 포함하고 있는 날의 마지막 날짜를 리턴
SELECT LAST_DAY(SYSDATE)
	FROM DUAL;

-- 사람의 사원번호, 사원이름, 입사일, 입사일 + 100일의 날짜, 입사일 +10년의 날짜 조회
SELECT E.ENO 
	 , E.ENAME
	 , E.HDATE 
	 , E.HDATE + 100	
	 , ADD_MONTHS(E.HDATE, 12 * 10)
	FROM EMP E;
